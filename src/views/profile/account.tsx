...
const Account: React.FC = () => {
  ...
  const handleChangeBtn = async ({ target }: any) => {
    const formData = new FormData();
    formData.append("file", target.files[0]);
    formData.append("address", address as `0x${string}`);
            ...
            <CardContent className='p-0 px-[24px]'>
              <div className='mb-[24px]'>
                <div className="relative">
                  <Image
                    // src={`/uploads/avatar/${address}.jpg`}
                  ...
                  {/* <input
                    className="absolute hidden-input opacity-0 cursor-pointer"
                    type="file"
                    onChange={handleChangeBtn}
                    accept="image/jpeg" // Optional: Limit file types to images
                  /> */}
                </div>
                <Typography gutterBottom variant="h3" component="div" className='m-0 font-bold text-[20px] md:text-[24px] text-white'>
                  Darlene Robertson
